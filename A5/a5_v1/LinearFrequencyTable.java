...
public class LinearFrequencyTable implements FrequencyTable {
    ...
    public void init(String key){
        ...
        if(size==0){
            //System.out.println("creating with size 0");
            create(key);
        }
        else{ //(size!=0)
            //System.out.println("checking key");
            boolean result=keycheck(key);
            if(result==false){
    ...
    private boolean keycheck(String key){
        ...
        while (p.next!=null && p.next!=head){
            if(p.key==key){
                ...
                return false; //throw new IllegalArgumentException("key is already present");
            }
        ...
        if(p.key==key){
            //System.out.println("key found after loop");
                return false; //throw new IllegalArgumentException("key is already present");
        }
        else{
            return true;
           ...
           throw new IllegalStateException("IMPLEMENT THIS METHOD");
                ...
                //while(n.next!=null && n.next!=head){
                    if(n.key==key){
                        ...
                        n=n.next; //System.out.println("next");
                 }
    ...
    public LinkedList<String> keys(){
        if (size == 0) { throw new IllegalStateException("there is no keys to make a list"); }
            ...
            if(size==1){ 
                ...
                int result= ( (current).compareTo(previous) );
                    ...
                    if( result==1 || result==0){
                        ...
                        //l.add(counter-1,p.key);}
                        //System.out.println("check result"); 
                        Long min=current; int minIndex=0;
    ...
    private LinkedList<Long> obtainValuesFromKeys(){
        ...
        try{
            LinkedList<String> key = keys(); int keysize= key.size();
            for(int i=0; i< keysize; i++){
                long getCount=get( key.get(i) );
                countlist.add(i,getCount);
            }      
        }
        catch(IllegalStateException e){
            System.err.println("there is no keys to make a list");
        }
        return countlist;
    }
    ...
    public long[] values() {
        if (size==0){
           throw new IllegalStateException("there is no keys");
        }
        long[] array= new long[size];
        LinkedList<Long> countlist=obtainValuesFromKeys();
        for(int i=0; i<array.length; i++){
            //long countlist = get( countlist.get(i) );
    ...
    public long[] values_WRONG() {
        ...
        long [] array= new long[size];
        Elem p=head.next;  //System.out.println(p.count); System.out.println(p.next.count); System.out.println(p.next.next.count);
        int counter=0;
        while(p!=head && counter<size){
            ...
            if(counter==0){ 
                ...
                Long current=p.count;  Long previous=q.count;

                int result= ( (current).compareTo(previous) );   //System.out.println(result);
                if( result==1 || result==0){
    ...
    private String displayValues(long [] array){
        if (array==null || array.length==0){
           throw new UnsupportedOperationException("there is nothing to display");
        }
        String s="[";
        for (int i=0; i<array.length; i++){
            s+=array[i];
